...
async def get_command_stats(
    period: str = Query(
        "week", description="Period for statistics: day, week, month, year"
    ),
    current_user: Dict[str, Any] = Depends(get_current_user),
):
    ...
    try:
        ...
        return {
            ...
            "total_tokens": tokens_query.data[0]["total_tokens"]
            if tokens_query.data
            else 0,
        }
...
async def get_command_detail(
    command_id: str, current_user: Dict[str, Any] = Depends(get_current_user)
):
