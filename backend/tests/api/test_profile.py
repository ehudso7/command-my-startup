...
# Test data
updated_profile = {"email": "updated@example.com", "full_name": "Updated User"}
    ...
    with patch("routes.profile.get_current_user", mock_get_current_user):
        with patch("routes.profile.get_supabase_client", return_value=MockSupabase()):
            async with AsyncClient(app=app, base_url="http://test") as client:
                ...
                response = await client.put("/api/profile", json=updated_profile)
...
async def test_create_api_key():
    ...
    mock_created_key = {"id": "new-key-id"}
    ...
    with patch("routes.profile.get_current_user", mock_get_current_user):
        with patch("routes.profile.get_supabase_client", return_value=MockSupabase()):
            async with AsyncClient(app=app, base_url="http://test") as client:
                ...
                response = await client.delete("/api/profile/api-keys/key-123")
    ...
    with patch("routes.profile.get_current_user", mock_get_current_user):
        with patch("routes.profile.get_supabase_client", return_value=MockSupabase()):
            async with AsyncClient(app=app, base_url="http://test") as client:
                ...
                response = await client.delete("/api/profile/api-keys/nonexistent-key")
