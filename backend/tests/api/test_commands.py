...
async def test_execute_command_with_different_models():
    ...
    # Apply patches
    with patch("routes.commands.get_current_user", mock_get_current_user):
        with patch("routes.commands.generate_response", side_effect=mock_generate_response):
            async with AsyncClient(app=app, base_url="http://test") as client:
                # Test with GPT-4
...
async def test_execute_command_with_error():
    ...
    # Apply patches
    with patch("routes.commands.get_current_user", mock_get_current_user):
        with patch("routes.commands.generate_response", side_effect=Exception("AI service error")):
            async with AsyncClient(app=app, base_url="http://test") as client:
