...
export default function VoiceCommand({ onCommandProcessed }: VoiceCommandProps) {
  ...
  const toggleListening = () => {
    if (!isSpeechRecognitionSupported) {
      showToast({
        type: 'error',
        title: 'Not Supported',
        message: 'Speech recognition is not supported in this browser. Try using Chrome, Edge, or Safari.',
      });
      return;
    }
  ...
  const handleExecuteCommand = async (command: string) => {
    ...
    try {
      ...
      setError(error.message || 'An error occurred while processing your command');
    ...
    <div className="space-y-4">
      ...
      {transcript && (
        <div className="border border-gray-200 dark:border-gray-700 rounded-md p-3">
          <p className="text-sm font-medium text-gray-700 dark:text-gray-300">Your command:</p>
